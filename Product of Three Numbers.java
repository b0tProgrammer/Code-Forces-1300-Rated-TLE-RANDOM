...
//            Comparator<Pair> com = Comparator.comparingInt(i -> i.v1);
            ...
            while (testcases-- > 0) {
                /*
                    In this, we have to select three different numbers whose product is equal to the number given.
                    if you observe that we can say that numbers
                 */
                ...
                for(int i = 2; i*i <= num; i++) {
                    if(num%i == 0) {
                        ...
                        if(num/(i*f) == 1 || num/(i*f) == i || num/(i*f) == f || f == num) out.println("NO");
                        else {
                            out.println("YES");
                            out.println(i+" "+f+" "+(num/(i*f)));
                        }
                        print = true;
                        break;
                    }
                }
                if(!print) out.println("NO");
            }
            out.close();
        } catch (Exception e) {
            e.printStackTrace();
