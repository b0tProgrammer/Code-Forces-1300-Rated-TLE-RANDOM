    ...
    public static void main(String[] args) {
        try {
            ...
            while (testcases-- > 0) {
                ...
                /*
                    The question is straight forward and asked to find the maximum and possible with exactly k operations!

                    In each operation, we can choose any j and will do a bitwise (or) with that element and 2^j simply stating that the bit position in that number is turned to 1!

                    So, to maximize the and, we have to ensure that we can make all the bits from the end as set!

                    This can be done by counting the number of UnsetBits at that particular location, and we check whether the current k can be used to set all the unSet bits at that position!
                 */
                ...
                int[] unSetBitsAtThatPos = new int[31]; // to have the frequency on number of unSetBits at a particular location!
                ...
                for(int i = 0; i < n; i++) {
                    ...
                    for(int j = 0; j <= 30; j++) {
                        ...
                        unSetBitsAtThatPos[j] += ((num&(1<<j)) == 0 ? 1 : 0);
                    }
                ...
                for(int i = 30; i >= 0; i--) {
                    ...
                    if(unSetBitsAtThatPos[i] <= k) {
                        k -= unSetBitsAtThatPos[i];
                        ans += (1<<i);
                    }
                }
