    ...
    public static void main(String[] args) {
        try {
            ...
            while (testcases-- > 0) {
                ...
                // for 4 the max up to we will be going is : ;
                ...
                while(low <= high) {
                    ...
                    long mid = (low+high) >> 1; // this is the current level;
...
//                    out.println(mid);
                    ...
                    // if mid > n then there is an issue on adjusting the currentLevels start and end!

                    if(mid > n) {
                        long diff = mid-n;
                        long alreadyVis = sumN(n);
                        long extraCovered = sumN(n-1)-sumN(2*n-mid);
                        // what does extraCover signify :
                        // it states that how much extra we have cleared other than from the middle one!
                        levelStart = alreadyVis+extraCovered;
                        levelEnd = levelStart+(n-(mid%n));
                        levelStart++;
                    } else {
                        levelEnd = mid*(mid+1) >> 1;
                        levelStart = levelEnd-(mid-1);
                    }
                    if(k >= levelStart && k <= levelEnd) {
                        level = mid;
                        break;
                    }
                    else if(levelEnd < k) {
