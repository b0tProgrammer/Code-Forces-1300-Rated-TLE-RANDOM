...
public class Main {
    ...
    public static void main(String[] args) {
        try {
            ...
            while (testcases-- > 0) {
                /*
                    In this, the question was straight-Forward!
                    we have to find the total pairs such that satisfies the condition :
                    ai < i < aj < j
                    just counting the number of valid pairs satisfying the above condition!

                    The main thing is we can have a run of values of current valid pairs and again we have seen an another
                    valid pair then we will add the current pairs to the ans and update the recently visited valid pair;
                 */

                int siz = in.nextInt();
                int[] a = new int[siz+1]; // to be comfortable let's consider 1 based indexing!
                int[] pre = new int[siz+1];

                long pairs = 0;

                for(int i = 1; i <= siz; i++) {
                    a[i] = in.nextInt();
                    pre[i] = pre[i-1]+(a[i] < i ? 1 : 0);
                    if(i > a[i]) {
                        pairs += pre[max(0,a[i]-1)]; // just counting the number of valid pairs that are already counted satisfying the condition!
                    }
                }
