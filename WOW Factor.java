...
//            testcases = in.nextInt();
            ...
            while (testcases-- > 0) {
                ...
                /*
                    In this, we should have the counter of number of v's in front of the current 'o' and after the 'o'
                    The multiplication of them is the answer
                 */

                int siz = s.length();

                int[] pre = new int[siz];
                int[] suff = new int[siz];

                for(int i = 1; i < siz; i++) {
                    pre[i] += pre[i-1];
                    if(s.charAt(i) == s.charAt(i-1) && s.charAt(i) == 'v')
                        pre[i]++;
                }
