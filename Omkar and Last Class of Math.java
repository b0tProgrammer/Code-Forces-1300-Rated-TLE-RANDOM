    ...
    public static void main(String[] args) {
        try {
            ...
            while (testcases-- > 0) {
                ...
                /*
                    we are given an integer n,  we have to write it in form of 2 numbers such that the sum of those numbers is equal to the n
                    and LCM(a,b) should be minimum
                 */

                int num = in.nextInt();

                // we know that the lcm has to be minimised so, the GCD should be maximized to maximize that GCD we need to select k such that k is the largest proper factor of n
                // to choose that k, we need find the smallest prime factor divides that number so, we can say that n/(smallestPrimeFactor(n)) == k
                // then our answer is (k,n-k);

                // we have to find the biggest factor of that number n
                ...
                out.println(biggestFactor+" "+(num-biggestFactor));
