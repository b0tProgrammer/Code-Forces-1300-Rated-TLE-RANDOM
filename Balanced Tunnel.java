...
//            testcases = in.nextInt();
            ...
            while (testcases-- > 0) {
                ...
                /*
                    how will you find that the vehicle has overTaken another vehicle?


                    We use an arrray to store the indices where the entry elements are ocuured! And we will check for every vehicle outside the tunnel.
                    that the element is overTaken or not! This can be done by checking the current vehicles position with the vehicle's position in the entry array.
                    And if the current vehicles position is greater than the entry vehicles position it states that the current vehicle overtook the entry array element.
                    Else the next element should be in the order of the entry element
                 */

                int siz = in.nextInt();
                int[] entry = new int[siz+1];
                int[] entryPos = new int[siz+1];
                for(int i = 1; i <= siz; i++) {
                    entry[i] = in.nextInt();
                    entryPos[entry[i]] = i;
                }
                ...
                for(int i = 1; i <= siz; i++) {
                    ...
                    set.add(entryPos[currVal]);
                    if(entryPos[currVal] > toCome) {
                        overTaken++;
                    }
                    else if(entryPos[currVal] == toCome) {
                        while(!set.isEmpty() && set.contains(toCome)) {
                            set.remove(toCome);
                            toCome++;
                        }
                    }
                }
                out.println(overTaken);
            }
            out.close();
        } catch (Exception e) {
            e.printStackTrace();
