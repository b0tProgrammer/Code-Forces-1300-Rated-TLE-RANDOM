...
public class Main {
    ...
    public static void main(String[] args) {
        try {
            ...
            while (testcases-- > 0) {
                // we have to find the factors of that number and check whether the compiliment of that factor is in the range!
                ...
                /*
                    He want's choose only one type of package so it's simple
                 */
                ...
                for(int i = 1; i*i <= n; i++) {
                    if(n%i == 0) {
                        if(i <= k) toBuy = min(toBuy,(n/i));
                        if(n/i <= k) toBuy = min(toBuy,i);
                    }
                }
